...
{
  Persona * carnet = malloc (sizeof(Persona));
  strcpy(carnet->nombre, nombre);
  strcpy(carnet->rut, rut);
  carnet->edad = edad;
  return carnet;
}
...
Vector * crearVector(int n) 
{
  ...
  v->datos = ((int*) malloc (n * sizeof(Vector)));
  printf("capacidad %i, datos %i, largo %i\n", v->capacidad, *(v->datos), n);
  return v;
}
